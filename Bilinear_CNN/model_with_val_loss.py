...
def train_model(model=None, name_model='BCNN_Keras', gen_dir_train=None, gen_dir_valid=None, max_epoch=50, batch_size=9):
    ...
    hist = model.fit_generator(gen_dir_train, epochs=max_epoch, callbacks=list_callbacks, workers=3, verbose=1)
...
def save_histogram(hist, path):
    ...
    if not os.path.exists(path):
        os.makedirs(path)
    val_loss = now + "_loss.txt"
    val_cat_acc = now + "_cat_acc.txt"
    np.array(hist.history['loss']).dump(open(val_loss, 'w'))
    np.array(hist.history['categorical_accuracy']).dump(open(val_cat_acc, 'w'))
...
def runner():
    ...
    ###
    gen_dir_train, gen_dir_valid = pre_process.load_data(path_data_train='./images/splitted/train', path_data_valid='./images/splitted/valid', size_mini_batch=9)
